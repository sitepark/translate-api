...
class TranslatableTextNodeTest {
  ...
  void testWithArrayNode() {
    ...
    TextNode textNode = Mockito.mock(TextNode.class);
    when(textNode.asText()).thenReturn("Text");
    TranslatableTextNode translatableTextNode = TranslatableTextNode.create(jsonNode, 1, textNode);

    assertThat(
        "unexpected instance", translatableTextNode, instanceOf(TranslatableTextNodeOfArray.class));
  }
